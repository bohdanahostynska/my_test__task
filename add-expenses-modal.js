...
class AddExpensesModal extends Component {
  constructor(props) {
    ...
    let correction = null;
    if (this.props.correction_id) {
      correction = {
        value: this.props.correction_id,
      };
    }
  ...
  componentDidMount() {
    ...
    this.listenEditFaild = ExpensesActions.editExpenses.failed.listen((res) => {
      const data = JSON.parse(res["response"]);
      this.setState({
        waiting: false,
        message: {
    ...
    this.listenEditCompleted = ExpensesActions.editExpenses.completed.listen(
      () => {
        setTimeout(() => {
          this.setState({ close: true });
          AppStore.closeModal();
  ...
  componentWillUnmount() {
    ...
    if (typeof this.listenEditFailed == "function") {
      this.listenEditFailed();
    }
    if (typeof this.listenEditCompleted == "function") {
      this.listenEditCompleted();
    }
  }
  ...
  submit() {
    ...
    const account = this.state.account.value,
      ...
      correction = this.state.category.value,
      shipment =
    ...
    trackEvent("users", "working", "add-expenses", "edit-expenses");
  }
      ...
      <div>
        ...
        <Box>
          ...
          <Field>
            <SelectComponent
              name="correction"
              label="expenses-correction"
              onChange={this.onChangeSelect}
              value={this.state.category}
              list={ExpensesStore.getCategoryList}
              addItem={ExpensesActions.editCategory}
              load={ExpensesActions.loadCategory}
              creatable={true}
            />
          </Field>
          ...
          <Button.Group>
            <SubmitButton
              text="edit-expenses-btn"
              waiting={this.state.waiting}
              submit={this.submit(data["id"])}
            />
          </Button.Group>
